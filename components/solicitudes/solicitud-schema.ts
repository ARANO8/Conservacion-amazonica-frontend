...
export const formSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.viaticos && data.actividades) {
      data.viaticos.forEach((viatico, index) => {
        const planIndexes = viatico.planificacionIndexes || [];
        if (planIndexes.length > 0) {
          let maxDias = 0;
          let maxPersonas = 0;

          planIndexes.forEach((idx) => {
            const act = data.actividades[idx];
            if (act) {
              maxDias += act.cantDias || 0;
              maxPersonas +=
                viatico.tipoDestino === 'TERCEROS'
                  ? act.cantTerceros || 0
                  : act.cantInstitucion || 0;
            }
          });

          if ((viatico.dias || 0) > maxDias && maxDias > 0) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: `Los días (${viatico.dias}) superan el límite planificado (${maxDias})`,
              path: ['viaticos', index, 'dias'],
            });
          }

          if (
            (viatico.cantidadPersonas || 0) > maxPersonas &&
            maxPersonas > 0
          ) {
            ctx.addIssue({
              code: z.ZodIssueCode.custom,
              message: `Las personas (${viatico.cantidadPersonas}) superan el límite planificado (${maxPersonas})`,
              path: ['viaticos', index, 'cantidadPersonas'],
            });
          }
        }
      });
